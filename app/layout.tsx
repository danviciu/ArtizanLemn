...
export default async function RootLayout({
  ...
  const initialConsentDecision = isCookieConsentDecision(rawConsentDecision)
    ? rawConsentDecision
    : null;
  const consentDefaultBootstrapScript = `(() => {
  window.dataLayer = window.dataLayer || [];
  ...
  window.gtag("consent", "default", {
    ...
    analytics_storage: ${initialConsentDecision === "accepted" ? '"granted"' : '"denied"'},
    wait_for_update: 500,
  });
})();`;
  ...
  return (
    <html lang="ro">
      <head>
        <script
          dangerouslySetInnerHTML={{ __html: consentDefaultBootstrapScript }}
        />
        <script
